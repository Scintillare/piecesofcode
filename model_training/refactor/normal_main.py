...
class Main:
    ...
    def setup(self):
        ...
        adv_idx = self.testloader.batch_sampler.sampler.indices[0:100]
    ...
    def evaluate(self):
        ...
        self.model.eval()
        clean_probabilities, labels = common.test.test(self.model, self.testloader, cuda=True, return_labels=True)
        ...
        adversarial_perturbations, adversarial_probabilities, adversarial_errors = common.test.attack(self.model, self.adversarialloader,
                                                                  attack, objective, attempts=1, cuda=True)

        test_labels = np.array(self.testloader.dataset.targets)[self.testloader.batch_sampler.sampler.indices]
        eval = common.eval.AdversarialEvaluation(clean_probabilities, adversarial_probabilities,
                                                    list(test_labels), validation=0.5, errors=adversarial_errors)
